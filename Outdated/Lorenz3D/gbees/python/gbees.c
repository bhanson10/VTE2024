...
void BST_get_dcu(TreeNode* r, BST* P, Grid* G){
    if (r == NULL){
        return;
    }
    BST_get_dcu(r->left, P, G);
    BST_get_dcu(r->right, P, G);

    r->cell.dcu = 0; TreeNode* i_node; TreeNode* k_node; double dcu_p; double dcu_m; 
    for(int q = 0; q < DIM; q++){
        r->cell.ctu[q] = 0.0; 
