...
double* Lorenz3D(double* x, double* dx, Grid* G, Traj T){
    double* v = (double*)malloc(G->dim * sizeof(double)); 
...
int main(){
    ...
    Traj T = Traj_create(sizeof(coef)/sizeof(coef[0]), coef); // Inputs: (# of coefficients, coefficients)
