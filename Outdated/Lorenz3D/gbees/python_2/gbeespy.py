...
def run_gbees(ADV, G, M, T, P_DIR, M_DIR, NUM_DIST, NUM_MEAS, DEL_STEP, OUTPUT_FREQ, DIM, OUTPUT, RECORD, MEASURE):
    lib.BST_create.argtypes = None
    lib.BST_create.restype = BST
    lib.initialize_grid.argtypes = [ct.POINTER(BST), ct.POINTER(Grid), Meas, Traj]
    lib.initialize_grid.restype = None
    lib.normalize_tree.argtypes = [ct.POINTER(BST)]
    lib.normalize_tree.restype = None
    lib.get_tree_info.argtypes = [ct.POINTER(BST), ct.POINTER(Grid)]
    ...
    lib.initialize_grid(ct.pointer(P), ct.pointer(G), M, T)
    initialize_vuw(P.root, G, T, ADV)
    lib.normalize_tree(ct.pointer(P))
    ...
    for nm in range(NUM_MEAS):
        lib.get_tree_info(ct.pointer(P),  ct.pointer(G))
        finish = time.time()
        if OUTPUT:
        ...
        if RECORD:
            ...
            lib.record_data(P.root, P_PATH.encode('utf-8'), G, ct.c_double(0.0))
        ...
        while(abs(mt - M.T) > TOL):
            ...
            if RECORD:
                ...
                lib.record_data(P.root, P_PATH.encode('utf-8'), G, ct.c_double(tt + mt + rt))
                record_count += 1
        ...
        if MEASURE and nm < NUM_MEAS-1:
            ...
            lib.measurement_update_recursive(P.root, G, M);                                     
            lib.normalize_tree(ct.pointer(P));
            lib.prune_tree(ct.pointer(P), ct.pointer(G));
