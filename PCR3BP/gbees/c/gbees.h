...
void write_file(FILE* myfile, TreeNode* r, Grid G){
    ...
    if (r->cell.prob >= G.thresh) {
        fprintf(myfile, "%.6e", r->cell.prob);
        for (int i = 0; i < DIM; i++) {
            fprintf(myfile, " %.6e", G.dx[i] * r->cell.state[i] + G.center[i]);
        }
        fprintf(myfile, "\n");
    }
}
...
void get_dcu(TreeNode* r, BST* P, Grid* G){
    ...
    r->cell.dcu = 0; 
    TreeNode* i_node; TreeNode* k_node; double dcu_p; double dcu_m; 
    for(int q = 0; q < DIM; q++){
        r->cell.ctu[q] = 0.0; 
        k_node = r->cell.k_nodes[q];
        i_node = r->cell.i_nodes[q];
        ...
        if(i_node!=P->dead){
            ...
            for(int b = 0; b < DIM; b++){
                if (b!=a){
                    ...
                    if(j_node!=P->dead){j_node->cell.ctu[b] -= i_node->cell.w[a] * j_node->cell.u[b] * F;}
                    if(p_node!=P->dead){p_node->cell.ctu[b] -= i_node->cell.u[a] * p_node->cell.u[b] * F;}
                }
            }
...
void update_prob(TreeNode* r, Grid* G){
    ...
    r->cell.prob = fmax(r->cell.prob, 0.0); 
}
