...
double gauss_probability(double* x, double* mat) {
    double *cov_inv = (double*)malloc(DIM * DIM * sizeof(double));
    int *ipiv = (int*)malloc(DIM * sizeof(int));
    double log_det_cov = 0.0;
    int info;

    // Copy covariance matrix to cov_inv because LAPACKE_dgetrf will overwrite it
    for (int i = 0; i < DIM * DIM; i++) {
        cov_inv[i] = mat[i];
    }

    // Compute the LU decomposition of cov
    info = LAPACKE_dgetrf(LAPACK_ROW_MAJOR, DIM, DIM, cov_inv, DIM, ipiv);
    if (info != 0) {
        printf("Error in LU decomposition: %d\n", info);
        free(cov_inv);
        free(ipiv);
        return -1;
    }

    // Compute the determinant of cov from the LU decomposition
    for (int i = 0; i < DIM; i++) {
        log_det_cov += log(fabs(cov_inv[i * DIM + i]));
    }

    // Compute the inverse of the covariance matrix using the LU decomposition
    info = LAPACKE_dgetri(LAPACK_ROW_MAJOR, DIM, cov_inv, DIM, ipiv);
    if (info != 0) {
        printf("Error in computing the inverse: %d\n", info);
        free(cov_inv);
        free(ipiv);
        return -1;
    }

    // Compute the exponent part -0.5 * x' * cov_inv * x
    double *temp = (double*)malloc(DIM * sizeof(double));
    cblas_dgemv(CblasRowMajor, CblasNoTrans, DIM, DIM, 1.0, cov_inv, DIM, x, 1, 0.0, temp, 1);
    double exponent = -0.5 * cblas_ddot(DIM, x, 1, temp, 1);
    ...
    double norm_factor = pow(2 * M_PI, -0.5 * DIM) * exp(-0.5 * log_det_cov);

    // Compute the final Gaussian probability density
    double pdf = norm_factor * exp(exponent);
    return pdf;
}
/*==============================================================================
CLASS DEFINITIONS
...
void initialize_vuw(TreeNode* r, BST* P, Grid* G, Traj T){
    ...
    if(r->cell.new_f==0){
        ...
        double r1 = pow(pow(x[0]+T.mu,2)+pow(x[1],2),1.5);
        double r2 = pow(pow(x[0]-1+T.mu,2)+pow(x[1],2),1.5);

        double v[DIM] = {x[2], x[3], 2*x[3]+x[0]-(T.mu*(x[0]-1+T.mu)/r2)-((1-T.mu)*(x[0]+T.mu)/r1), -2*x[2]+x[1]-(T.mu*x[1]/r2)-((1-T.mu)*x[1]/r1)};
...
void initialize_grid(BST* P, Grid* G, Measurement M, Traj T){
    ...
    int max[DIM] = {INT_MAX, INT_MAX, INT_MAX, INT_MAX};
    ...
    for (int i = (int) round(-3*pow(M.cov[0][0],0.5)/G->dx[0]); i <= (int) round(3*pow(M.cov[0][0],0.5)/G->dx[0]); i++){current_state[0] = i; current_state_vec[0] = i*G->dx[0]; 
        for (int j = (int) round(-3*pow(M.cov[1][1],0.5)/G->dx[1]); j <= (int) round(3*pow(M.cov[1][1],0.5)/G->dx[1]); j++){current_state[1] = j; current_state_vec[1] = j*G->dx[1];
            for (int k = (int) round(-3*pow(M.cov[2][2],0.5)/G->dx[2]); k <= (int) round(3*pow(M.cov[2][2],0.5)/G->dx[2]); k++){current_state[2] = k; current_state_vec[2] = k*G->dx[2];
                for (int l = (int) round(-3*pow(M.cov[3][3],0.5)/G->dx[3]); l <= (int) round(3*pow(M.cov[3][3],0.5)/G->dx[3]); l++){current_state[3] = l; current_state_vec[3] = l*G->dx[3];
                    for (int m = (int) round(-3*pow(M.cov[4][4],0.5)/G->dx[4]); m <= (int) round(3*pow(M.cov[4][4],0.5)/G->dx[4]); m++){current_state[4] = m; current_state_vec[4] = m*G->dx[4];
                        for (int n = (int) round(-3*pow(M.cov[5][5],0.5)/G->dx[5]); n <= (int) round(3*pow(M.cov[5][5],0.5)/G->dx[5]); n++){current_state[5] = n; current_state_vec[5] = n*G->dx[5];
