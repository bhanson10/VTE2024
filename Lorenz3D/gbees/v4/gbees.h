...
void update_ctu(LL* P, Grid* G){
    Node* r = P->head;
    while (r != NULL) {
        for(int a = 0; a < DIM; a++){
            Node* i_node = r->cell.i_nodes[a];
            Node* j_node; Node* p_node;
            if(i_node!=P->dead){
                ...
                if (i_node->cell.v[a]>0){
                    Node* i_i_node = i_node->cell.i_nodes[a];
...
void godunov_method(LL* P, Grid* G){
    get_dcu(P, G);
    update_ctu(P, G);
}

void update_prob(LL* P, Grid* G){
    Node* r = P->head;
    while (r != NULL) {
